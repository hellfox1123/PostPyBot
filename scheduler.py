...
class Scheduler:
    ...
    async def _publish_post(self):
        """Публікація поста (викликається планувальником)"""
        try:
            # Перевіряємо розклад
            ...
            # Отримуємо наступний пост
            post_id = await poster.get_next_post()
            ...
            # Копіюємо пост
            success = await poster.copy_post(post_id)
            
            if success:
                logger.info(f"Пост {post_id} успішно опубліковано")
            else:
                logger.warning(f"Пост {post_id} не вдалося опублікувати")
    ...
    async def _is_in_schedule(self) -> bool:
        ...
        current_day = now.isoweekday()  # 1 = понеділок, 7 = неділя
